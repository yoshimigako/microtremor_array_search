...
PENALTY_INSIDE_NOISY_SCALE = ctrl.get("PENALTY_INSIDE_NOISY_SCALE", 1.0)
...
print(f"PENALTY_INSIDE_NOISY_SCALE : {PENALTY_INSIDE_NOISY_SCALE}")


CSV_PRIMARY = "array_primary_results.csv"
CSV_BEST = "array_best_by_radius.csv"
CSV_PENALTY = "array_penalty_breakdown.csv"
CSV_SELECTED = "array_selected_points.csv"
KML_SELECTED = "array_selected_points.kml"
...
pd.DataFrame(columns=[
    ...
    "lat",
    "lon"
]).to_csv(CSV_SELECTED, index=False)
...
def inside_noisy_penalty(pts, R):
    """
    三角形アレイ内部に含まれる「幹線道路・鉄道（noisy_u）」の量に基づくペナルティ。
    - R >= 100 m の場合のみ適用（ユーザー要件）
    - noisy_u の三角形内の交差部分の総延長 [m] を用いる（線状ノイズ源に対して直感的）
    - 半径で正規化（Rが大きいほど必然的に延長が増えるため）
    """
    if R < 100:
        return 0.0
    try:
        tri = Polygon([(p.x, p.y) for p in pts])
        inter = noisy_u.intersection(tri)
        if inter.is_empty:
            return 0.0
        L = inter.length  # [m]
        return (L / max(R, 1.0)) * PENALTY_INSIDE_NOISY_SCALE
    except Exception:
        # 何らかの幾何演算エラーがあっても探索を止めない（ペナルティ0扱い）
        return 0.0
...
def triangle_error(center, pts, R):
    ...
    return d_err + R * np.radians(ang_err) + env_pen + inside_noisy_penalty(pts, R)
...
for seed in seeds:
    ...
    for i, c0 in enumerate(centers, 1):
        ...
        if valid:
            ...
            for R, lst in valid.items():
                total_err += min(e for _, _, e in lst)
            scored.append(((len(valid), total_err), c0, valid))
    ...
    for rank,(_,c0,valid) in enumerate(top,1):
        ...
        for R,lst in valid.items():
            ...
            if rank == 1:
                selected_rows.append([sid, R, "center", 0, lat, lon])
                for i, p in enumerate(pts, 1):
                    lo, la = to_ll.transform(p.x, p.y)
                    selected_rows.append([sid, R, "observation", i, la, lo])
    ...
    pd.DataFrame(selected_rows).to_csv(CSV_SELECTED, mode="a", header=False, index=False)
    m.save(MAP_FILE)
    print(f"\n✔ {sid}: CSV / 地図 更新完了")

def write_kml(path, rows):
    head = [
    ...
    tail = ["</Document>", "</kml>"]
    body = []
    for seed_id, radius, point_type, point_id, lat, lon in rows:
        name = f"{seed_id}_R{radius}_{point_type}{point_id}"
        body.append("<Placemark>")
        body.append(f"<name>{name}</name>")
        body.append("<Point>")
        body.append(f"<coordinates>{lon},{lat},0</coordinates>")
        body.append("</Point>")
        body.append("</Placemark>")
...
try:
    ...
    write_kml(KML_SELECTED, df_sel.values.tolist())
    print(f"\n✔ 選定点KML出力: {KML_SELECTED}")
